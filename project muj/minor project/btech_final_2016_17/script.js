...
var field_arr = new Array('text','text','text','text');
  ...
  var field_name = new Array('fname','lname','fcode','email'); 
 $(function(){
 $.ajax({
 ...
 $('#demoajax').on('click','.ajaxsave',function(){
	   ...
	   var fcode =$("input[name='"+field_name[2]+"']");
	   var email = $("input[name='"+field_name[3]+"']");
	   if(validate(fname,lname,fcode,email)){
	   data = "fname="+fname.val()+"&lname="+lname.val()+"&fcode="+fcode.val()+"&email="+email.val()+"&actionfunction=saveData";
       $.ajax({
	     url:"DbManipulate.php",
                  type:"POST",
                  data:data,
 ...
 $('#demoajax').on('click','.ajaxedit',function(){
      var edittrid = $(this).parent().parent().attr('id');
	   ...
	   var email = $("input[name='"+field_name[3]+"']");
	   if(validate(fname,lname,fcode,email)){
	   data = "fname="+fname.val()+"&lname="+lname.val()+"&fcode="+fcode.val()+"&email="+email.val()+"&editid="+edittrid+"&actionfunction=updateData";
       $.ajax({
	     url:"DbManipulate.php",
                  type:"POST",
                  data:data,
...
function validate(fname,lname,fcode,email){
var contact = jQuery('input[name=contact]');
		...
		if (fcode.val()=='') {
			fcode.addClass('hightlight');
			return false;
		} else fcode.removeClass('hightlight');
