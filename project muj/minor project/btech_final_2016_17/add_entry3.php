...
echo $today_time=date("H:i:s");
if (($today_time>="08:59:00")&&($today_time<="17:00:00"))
{
	$block_entry=0;
}
else
...
if ($block_entry==1)
{
	$entered_people=get_entry_list();
	$aaa=sizeof($entered_people);
	if ($aaa != 0) 
	{
    		...
    		for ($i=0; $i<$aaa; $i++)
		{
			$enroll= $entered_people[$i]['enroll_no'];
			$qry="UPDATE student SET prev_entry_status=1 WHERE enroll_no=".$enroll.";";
			mysql_query($qry);
			$query="delete from entry_details where enroll_no=".$enroll.";";
			mysql_query($query); 
...
<?php
if (($today_time>="08:50:00")&&($today_time<="17:00:00"))
{
	$block_entry=0;
	?>
...
else
{
	...
	<table id='formtable'  cellspacing="0">
<tr>
<td><span class="notice">
You are not allowed to Login during this Time</span></td>
</tr>
</table>
<?php
}
