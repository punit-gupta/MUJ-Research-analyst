  ...
  <?php if($_REQUEST[op]<100) { ?>
   <td width="160" align="center" bgcolor="#EEEEEE" style="padding: 10pt 10pt 10pt 10pt">
    <?php if($_SESSION[DBN]) { ?>
     ...
     <form action="<?php echo($WIZ); ?>?op=12" method="post">
      <b>.: CREATE TABLE :.</b><br>
      Table name:<br>
...
case "2":
 ...
 for($i=0;$i<$cols;$i++) {
  ...
  echo("<tr style='background-color:$bgcolor' onMouseOver='javascript:this.style.backgroundColor=\"#CCFF00\"' onMouseOut='javascript:this.style.backgroundColor=\"$bgcolor\"'>
   ...
   <td $BORDER>$col_info[1]</td>
   <td $BORDER>$col_info[2]</td>
   <td $BORDER>$col_info[4]</td>
   <td $BORDER>$col_info[5]</td>
 ...
 if(strtolower(substr($_POST[qr],0,6))=="select" || (!$_POST[qr] && $_SESSION[select])) {
  ...
  if($rs=@mysql_query($_SESSION[select],$dbl)) {
   ...
   while($fn[$i]=@mysql_field_name($rs,$i)) {
    echo("<th style='border:1pt dotted #999999;border-collapse:collapse;'><a href='$WIZ?op=4&order=$fn[$i]'>$fn[$i]</a></th>");
    $i++;
   }
   echo("</tr>");
   while($rc=mysql_fetch_row($rs)) {
...
<table cellspacing="0" cellpadding="0" width="100%" border="0" valign="middle"><tr height="20">
 <th bgcolor="#CCCCCC">Copyright &copy; 2004 Marco Avidano -:- Powered by <a href="<?php echo($VER[URL]); ?>" target='_blank'><?php echo($VER[WEB]); ?></a></td>
</tr></table>
</body>
</html>
<?php
