		...
		if ($enroll_exists>0)
		{
			...
			if($entered_password == $password)
			{
				...
				if ($already_in==1)
				{
					...
					$in_date=$student_entry_details['indate'];
				...
				else
				{
					...
					else
					{
						...
						//$error ="YOUR TOTAL ATTENDANCE : ".$view_duration['hours']." hours"." ".$view_duration['minutes']." minutes out of total hours"; 
						unset($_POST);
						mysql_close($connection);// Closing Connection
...
<!--
<tr>
<td colspan='2'><span style="font-color:red">
<marquee behavior="slide"><b><u>Shashwat Singh</u> (123207), <u>Deepankar Jain</u> (121318) are required to contact the admin at the earliest</b></marquee>
</span>
...
if ($aaa != 0) {
    ...
    for ($i=0; $i<$aaa; $i++)
	{
		...
		$stu_pass = get_student_password_by_id($view_enroll);
		...
		<?php
       // echo "<tr><td>".$j."</td><td><a href='view_profile.php?enroll=".$view_enroll."' target='_blank'>".$view_enroll."</a></td><td>".$view_name."</td><td>".$t."</td></tr>";
 echo "<tr><td>".$j."</td><td><a href='view_profile.php?enroll=".$view_enroll."' target='_blank'>".$view_enroll."</a></td><td>".$view_name."</td><td>".$t."</td></tr>";		?>
 
<!--<a href='view_profile.php?enroll=".$view_enroll."' target='_blank'></a>-->
		</form>
		<?php
    }
