...
if (isset($_GET['entry_no'])) 
{
	...
	if ($action==1)
	{
		...
		$login_ter=$e_data['login_terminal'];
		$time_spent=get_timediff_in_sec($in_time,$date_time);
		
		$qry2="insert into full_entry_details(enroll_no, indate, in_time, out_time, duration, login_terminal, logout_terminal, lab) VALUES ($enroll, '$today_date', '$in_time', '$date_time', $time_spent, '$login_ter', '$client_ip_addr','CL9');";
					mysql_query($qry2);
					$qry1="delete from entry_list WHERE enroll_no=".$enroll.";";
					mysql_query($qry1);					
					$qry="delete from entry_details WHERE sr_no=".$entry_no.";";
					mysql_query($qry);
	...
	{	
		...
		$enroll=$_GET['enroll'];
		$date_time=get_curdate_time();
		$query="delete from entry_details where sr_no=$entry_no";
		mysql_query($query); 
...
?>
<?php if ($a==2) {?><td colspan='3'>Action</td><?php } //when current entries?>
</tr>
<?php
...
if ($aaa != 0) {
    ...
    for ($i=0; $i<$aaa; $i++)
	{
		...
		if ($a==1)
		{
			$view_interminal = $entry_data[$i]['login_terminal'];
			$view_outterminal = $entry_data[$i]['logout_terminal'];
			if ($view_outterminal == "")
				$view_duration=="";
			else if ($view_outterminal <> "")
